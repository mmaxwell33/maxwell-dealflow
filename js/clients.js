...
const Clients = {
  ...
  async openDetail(id) {
    ...
    if (!c) return;
    // Get viewings count for this client
  ...
  openEdit(id) {
    ...
    App.openModal(`
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;">
        <button class="btn btn-red" onclick="Clients.delete('${c.id}','${c.full_name}')">🗑 Archive</button>
  ...
  async delete(id, name) {
    if (!confirm(`Archive ${name}? They will still appear in historical data.`)) return;
    await db.from('clients').update({ status: 'Inactive' }).eq('id', id);
    App.closeModal(); App.toast('Client archived.');
    Clients.load(); App.loadOverview();
  }
};
