// Maxwell DealFlow CRM — backup-data edge function (REFINED)
//
// What changed vs. the original:
//   • GMAIL_USER no longer has a hardcoded 'maxwelldelali22@gmail.com'
//     fallback. If the secret isn't set the function returns 500 with
//     a clear message — no "silent send to the wrong mailbox" risk.
//   • Backup email now goes to AGENT_EMAIL (if set) so the login address
//     and the backup recipient can be different if that ever matters.
//     Falls back to GMAIL_USER, which preserves today's behaviour.
//
// Required Supabase secrets (no change in count):
//   GMAIL_USER            — the Gmail account that sends the backup
//   AGENT_EMAIL           — where to deliver the backup (optional; defaults to GMAIL_USER)
//   GMAIL_CLIENT_ID       — OAuth client
//   GMAIL_CLIENT_SECRET   — OAuth secret
//   GMAIL_REFRESH_TOKEN   — OAuth refresh token
//   SUPABASE_SERVICE_ROLE_KEY — used to bypass RLS while reading everything to back up
...
serve(async (req) => {
  ...
  try {
    ...
    return new Response(JSON.stringify({
      ...
      message: `Backup emailed to ${AGENT_EMAIL}`,
